...
class SparseBayesianMC(BaseInference):
    ...
    def preprocess_data(self, X):
        ...
        all_feature_names = np.concatenate(
            [poly.get_feature_names_out(self.data_names), trig_feature_names]
        )
        ...
        all_features = all_features[
            :-1
        ]  # Exclude the last row to match the target shape
    ...
    def fit(self, X, y):
        ...
        with pm.Model(coords=coords) as model:
            ...
            likelihood = pm.Normal(
                "y", mu=mu, sigma=sigma, observed=y, dims=("data_points", "targets")
            )

            # Sampling
            idata = pm.sample(3000, return_inferencedata=True)
