...
def animate_temporal_graph(
    ...
    **kwargs,
    ...
    Args:
        ...
        node_sizes: Optional array of shape (T, N) or (N,) for dynamic/static node sizes.
    ...
    if node_sizes is not None and hasattr(node_sizes, "device_buffer"):
    ...
    if node_sizes is not None:
        if node_sizes.ndim == 2:
            initial_sizes = node_sizes[0]
        else:
            initial_sizes = node_sizes
    else:
        initial_sizes = 100
    if D == 2:
        scat = ax.scatter(
            pos[0, :, 0], pos[0, :, 1], c=initial_colors, s=initial_sizes, zorder=2
        )
    else:
        scat = ax.scatter(
            pos[0, :, 0], pos[0, :, 1], pos[0, :, 2], c=initial_colors, s=initial_sizes
        )
