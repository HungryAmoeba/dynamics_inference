...
class BlenderVisualizer:
    ...
    def visualize(self, pos, ori=None, save_path=None, **kwargs):
        """
        Visualize particle motion using Blender.
        Args:
            pos: Array of shape (T, N, 3)
            ori: Array of shape (T, N, 4) (quaternions) or (T, N, 3) (vectors)
            save_path: Path to save the rendered animation, if specified
            **kwargs: Additional arguments (currently unused)
        Returns:
            None
        """
        return animate_particle_motion_blender(pos, ori, save_path=save_path)


def animate_particle_motion_blender(
    pos: np.ndarray, ori: np.ndarray, save_path: str = None
):
    """
    ...
    with (
        tempfile.NamedTemporaryFile(suffix="_pos.npy", delete=False) as pos_file,
