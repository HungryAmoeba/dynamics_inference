...
class DynamicalSystem(ABC):
    ...
    def unwrap_state(self, state: jnp.ndarray) -> Dict[str, jnp.ndarray]:
        """
        Unwrap the state vector into its constituent parts as a dictionary.

        Args:
            state: State vector to unwrap

        Returns:
            Dictionary mapping component names to arrays (e.g., positions, orientations, volumes)
        """
        pass
...
class InteractingParticleSystem(DynamicalSystem):
    ...
    def unwrap_state(self, state: jnp.ndarray) -> Dict[str, jnp.ndarray]:
        ...
        Returns:
            ...
            - 'orientations': (N, D) or (T, N, D) array for orientations
        """
