...
class DynamicsEvaluator(BaseEvaluator):
        ...
        if system is not None:
            ...
            for i, state in enumerate(trajectory):
                ...
                if hasattr(system, "unwrap_state"):
                    unwrapped = system.unwrap_state(deriv.reshape(1, -1))
                    if isinstance(unwrapped, dict):
                        # New dictionary format
                        pos = unwrapped.get("positions", jnp.array([]))
                        ori = unwrapped.get("orientations", jnp.array([]))
                        if len(pos) > 0 and len(ori) > 0:
                            deriv_reshaped = jnp.concatenate([pos[0], ori[0]])
                        elif len(pos) > 0:
                            deriv_reshaped = pos[0]
                        else:
    ...
    def compute_metrics(
        self, true_derivatives: jnp.ndarray, pred_derivatives: jnp.ndarray, model
    ) -> Dict[str, float]:
        """
            ...
            for grade in jnp.unique(g_of_d):
                ...
                if len(grade_idxs) > 0:
                    grade_mse = float(
                        jnp.mean(
                            (
                                true_derivatives[..., grade_idxs]
                                - pred_derivatives[..., grade_idxs]
                            )
                            ** 2
                        )
                    )
    ...
    def generate_figures(
        ...
        fig_path = self.plot_parity(true_derivatives, pred_derivatives, output_dir)
    ...
    def plot_trajectory_comparison(
        ...
        for i, p in enumerate(particle_idxs):
            for j, c in enumerate(component_idxs):
                ax = axes[i, j]
                ax.plot(
                    times, true_derivatives[:, p, c], "b-", label="True", linewidth=2
                )
                ax.plot(
                    times,
                    pred_derivatives[:, p, c],
                    "r--",
                    label="Predicted",
                    linewidth=1,
                )
