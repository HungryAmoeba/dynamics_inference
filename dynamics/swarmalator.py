...
class Swarmalator(DynamicalSystem):
    def initialize(self, config, key=jrandom.PRNGKey(0)):
        ...
        self.orientations = self.orientations / jnp.linalg.norm(self.orientations, axis=1, keepdims=True)
    ...
    def return_state(self):
        return jnp.concat([self.positions.ravel(), self.orientations.ravel()])  # Shape: (2ND,)
    ...
    def compute_derivatives_unreshaped(self, state):
        N = self.N
        alpha, beta, gamma, J, R, epsilon_a, epsilon_r = self.alpha, self.beta, self.gamma, self.J, self.R, self.epsilon_a, self.epsilon_r
        ...
        pos_diffs = positions[None, :, :] - positions[:, None, :]  # Now (N, N, D) with x_j - x_i
        min_distance = 1e-4
        ...
        force_matrix = ((spatial_attraction - spatial_repulsion) * mask)[..., None] * pos_diffs
        ...
        sigma_projection = orientations[None, :, :] - (orientation_dot[..., None] * orientations[:, None, :])
    ...
    def compute_derivatives_unreshaped_naive(self, state):
        ...
        for i in range(N):
            ...
            for j in range(N):
                ...
                pos_deriv[i] += (1 + J * dot_oris) * displacement_ji/ norm_alpha - displacement_ji / norm_beta
