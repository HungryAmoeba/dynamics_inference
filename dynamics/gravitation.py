...
class GravitationalSystem(DynamicalSystem):
    ...
    def initialize(self, config):
        ...
        pos_range = config.get('initial_conditions', {}).get('position_range', [-1.0, 1.0])
        vel_range = config.get('initial_conditions', {}).get('velocity_range', [-0.1, 0.1])
        # Use provided RNG key or a default one
        self.rng = jrandom.PRNGKey(config.get('rng_key', 0)) if isinstance(config.get('rng_key'), int) else \
        config.get('rng_key', jrandom.PRNGKey(0))
        
        # Create positions and velocities randomly
        positions = jrandom.uniform(self.rng, (self.num_particles, self.dimension),
                                   minval=pos_range[0], maxval=pos_range[1])
        self.rng, rng_vel = jrandom.split(self.rng)
        velocities = jrandom.uniform(rng_vel, (self.num_particles, self.dimension),
    ...
    def compute_derivatives(self, t, state, args):
        ...
        def acceleration_on_particle(i):
            ...
            acc = self.G * jnp.sum(self.masses[:, None] * disp * inv_dist3[:, None], axis=0)
            return acc

        # Vectorize the acceleration computation for all particles.
        acceleration = jax.vmap(acceleration_on_particle)(jnp.arange(self.num_particles))
    ...
    def _unwrap_state(self, state):
        ...
        """
        positions = state[:self.num_particles * self.dimension].reshape(self.num_particles, self.dimension)
        velocities = state[self.num_particles * self.dimension:].reshape(self.num_particles, self.dimension)
    ...
    def unwrap_state(self, state):
        ...
        """
        positions = state[:, :self.num_particles * self.dimension].reshape(-1, self.num_particles, self.dimension)
